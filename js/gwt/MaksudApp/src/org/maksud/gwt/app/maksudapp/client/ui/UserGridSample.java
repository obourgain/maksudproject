...
public class UserGridSample extends LayoutContainer {
	...
	protected void onRender(Element parent, int index) {
		...
		BasicRPC.Util.getInstance().getUsers(new AsyncCallback<List<UserModel>>() {
			...
			public void onSuccess(List<UserModel> result) {
				...
				{
					final ListStore<UserModel> store = new ListStore<UserModel>();
					store.add(result);
					...
					ColumnModel cm = new ColumnModel(configs);
					Grid<UserModel> grid = new Grid<UserModel>(store, cm);
					grid.setStyleAttribute("borderTop", "none");
